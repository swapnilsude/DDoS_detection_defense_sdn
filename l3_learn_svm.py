...
class l3_switch (EventMixin):
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      my_packet = str(packet.next)
      protocol = 0
      if 'UDP' in my_packet:
          protocol = 17
      elif 'ICMP' in my_packet:
          protocol = 1
      elif 'TCP' in my_packet:
          protocol = 6
     
      class_packet = 0 
      if packet.next.ttl < 50 :
          class_packet = '1'
      else:
          class_packet = '0'
      # we are only using ttl as label/target_variable: Y_train but not to train it as a classifing data. 
      # The dataset: X_train is (currentTime, source IP, dst IP, protocol, etc)
      # currently we are still figuring out to use more attributes in X_train
      self.svm.classify([currentTime, sourceIP, destinationIP, protocol, class_packet])
